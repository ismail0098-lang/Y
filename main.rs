            ...
            if target_file.exists() {
                if let Ok(canonical) = fs::canonicalize(&target_file) {
                    if imported_files.insert(canonical) {
                        println!("[*] Loading imported module: {}", target_file.display());
                        match fs::read_to_string(&target_file) {
                            ...
                            Err(e) => {
                                eprintln!("[!] Failed to read imported file {}: {}", target_file.display(), e);
                                exit(1);
                            }
                        }
                    }
                ...
                println!("[*] Warning: Imported module file not found: {}", target_file.display());
            }
        }
        index += 1;
    }
