...
# STRUCT-OF-ARRAYS. Limb `j` of element `i` lives at `[j * count + i]`, not at
# `[i * 8 + j]`.
#
# This is not a preference, it is the single biggest thing in these kernels,
# and NCU is what made it visible. Array-of-structs puts the 32 threads of a
# warp 32 BYTES apart for any fixed limb, so one `ld.global.u32` asks for 32
# separate sectors and uses four bytes of each. L1 absorbs the repeat traffic
# (84% hit rate) so the DRAM figure looked fine, but the sector REQUESTS still
# queue: the profiler showed 45.1 of every 64.3 cycles stalled on the
# local/global instruction queue being full, at 9.73% compute throughput.
#
# Transposed, a warp's 32 consecutive `u32` are 128 contiguous bytes - four
# sectors, one transaction. Eight times fewer requests for identical work.
def load(var, buf, idx, count, ind="    "):
    return [f"{ind}let {var}{j}: U32 = block_ptr2d_load({buf}, {j}, {idx}, "
            f"{count}, 8, {count});" for j in range(S)]


def store(var, buf, idx, count, ind="    "):
    return [f"{ind}block_ptr2d_store({buf}, {j}, {idx}, {count}, 8, "
            f"{count}, {var}{j});" for j in range(S)]
