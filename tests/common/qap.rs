...
use super::msm::{as_bytes, load_kernel};
...
impl GpuQap {
    pub fn new(ctx: &CudaContext, domain_size: usize) -> Self {
        let n = domain_size;
        let log_n = n.trailing_zeros();
        assert!(n.is_power_of_two(), "the evaluation domain must be a power of two");
        let r = r_mod_r();
        ...
        let upload = |v: &[ArkFr]| {
            let flat = to_planar_mont(v, v.len(), r);
            let d = ctx.alloc(flat.len() * 4).unwrap();
            ctx.memcpy_htod_at(&d, 0, as_bytes(&flat)).unwrap();
            d
        };

        // Stage `s` needs `w_m^pos` for pos in 0..2^(s-1), where w_m = w^(n/m).
        let build_tw = |root: ArkFr| -> Vec<DeviceBuffer> {
            (1..=log_n)
                .map(|s| {
                    let m = 1usize << s;
                    let half = m / 2;
        ...
        let d_perm = {
            let d = ctx.alloc(perm.len() * 4).unwrap();
            ctx.memcpy_htod_at(&d, 0, as_bytes(&perm)).unwrap();
            d
        };
        ...
        GpuQap {
            ...
            r,
            r_inv,
            stage: load_kernel(ctx, "bn254_ntt_stage"),
            permute: load_kernel(ctx, "bn254_permute"),
            mul: load_kernel(ctx, "bn254_fr_mul_fast"),
            sub: load_kernel(ctx, "bn254_sub_vec"),
            tw_fwd: build_tw(w),
            tw_inv: build_tw(w_inv),
            d_perm,
            d_t1: upload(&pow_table(g, n_inv)),
            d_t2: upload(&pow_table(g_inv, n_inv * vanishing)),
            d_a: ctx.alloc(bytes).unwrap(),
            d_b: ctx.alloc(bytes).unwrap(),
            d_c: ctx.alloc(bytes).unwrap(),
            d_scratch: ctx.alloc(bytes).unwrap(),
        }
    }

    fn launch(&self, ctx: &CudaContext, m: &KernelModule, threads: usize, args: &[u64]) {
    ...
    /// device, into `dst`.
    fn transform(&self, ctx: &CudaContext, src: &DeviceBuffer, dst: &DeviceBuffer, inv: bool) {
        self.launch(
            ctx,
            &self.permute,
            self.n,
    ...
    fn pointwise(&self, ctx: &CudaContext, m: &KernelModule, a: &DeviceBuffer, b: &DeviceBuffer, out: &DeviceBuffer) {
        self.launch(
            ctx,
            m,
            self.n,
    ...
    fn ifft_then_coset_fft(&self, ctx: &CudaContext, x: &DeviceBuffer) {
    ...
    pub fn h_from_abc(&self, ctx: &CudaContext, a: &[ArkFr], b: &[ArkFr], c: &[ArkFr]) -> Vec<ArkFr> {
        assert_eq!(a.len(), self.n);
        let up = |v: &[ArkFr], d: &DeviceBuffer| {
            let flat = to_planar_mont(v, self.n, self.r);
            ctx.memcpy_htod_at(d, 0, as_bytes(&flat)).unwrap();
        };
        up(a, &self.d_a);
        up(b, &self.d_b);
        ...
        ctx.synchronize().expect("the QAP chain did not complete");

        let mut raw = vec![0u8; self.n * 8 * 4];
        ctx.memcpy_dtoh_at(&mut raw, &self.d_scratch, 0).unwrap();
        let w: Vec<u32> = raw
            .chunks_exact(4)
            .map(|x| u32::from_le_bytes([x[0], x[1], x[2], x[3]]))
            .collect();
        from_planar_mont(&w, self.n, self.r_inv)
    }
}
