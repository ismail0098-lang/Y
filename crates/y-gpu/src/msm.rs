//! Pippenger multi-scalar multiplication over BN254 G1, on the GPU.
...
use crate::device::{alloc, read_u32, sync, upload_u32};
...
// shared the destination, and how many POINTS each of them owned. Read through
// `last_scatter_shape`, which carries the doc comment.
//
// Recorded by `scatter` itself rather than recomputed by the caller. Both
// values are already derived there and re-deriving them anywhere else would be
// a second copy of `ceil(nchunk / group)` — the defect
// `proofs/ExactGemmSchedule.v` exists to remove, in the harness instead of the
// emitter. `tests/msm_counting_sort_model.rs` needs them to evaluate the
...
/// The grid and block `bn254_msm_bucket` is launched with: one thread per
/// bucket, in blocks of [`bucket_block`].
///
/// Split out as a pure function so the launch decision can be checked without
/// a device. It is the whole reason the block size is a knob at all, and it is
/// a PERFORMANCE property, which this repository has repeatedly found needs a
/// guard of its own — a wrong block size here computes the right answer, so no
/// correctness test can see it. `the_bucket_kernel_is_launched_at_the_tuned_
/// block` in `tests/zk_gpu_msm.rs` is that guard.
///
/// The grid divides exactly because `Geom::new` asserts
/// `nb % bucket_block() == 0`.
pub fn bucket_launch_geometry(g: &Geom) -> ((u32, u32, u32), (u32, u32, u32)) {
    let b = bucket_block();
    debug_assert_eq!(g.nb % b, 0, "Geom::new should have refused this bucket count");
    (((g.nb / b) as u32, 1, 1), (b as u32, 1, 1))
}
...
pub fn gpu_msm(
    ...
    scalars: &[Fr],
    g: &Geom,
) -> Result<(G1Projective, MsmTiming)> {
    let (buckets, _off, tm) = bucket_pass(ctx, module, points, scalars, g)?;
    Ok(reduce_buckets(&buckets, g, tm))
}
...
pub fn bucket_pass(
    ...
    scalars: &[Fr],
    g: &Geom,
) -> Result<(Vec<G1Projective>, Vec<u32>, MsmTiming)> {
...
pub struct DeviceBases {
    pub dpx: DeviceBuffer,
    pub dpy: DeviceBuffer,
    pub dpz: DeviceBuffer,
    pub n: usize,
    pub stage: f64,
    pub h2d: f64,
}

pub fn stage_bases(ctx: &CudaContext, points: &[G1Projective]) -> Result<DeviceBases> {
    use std::time::Instant;
    let r = r_mod_q();
    let t = Instant::now();
    let (hx, hy, hz) = (
    ...
    sync(ctx)?;
    Ok(DeviceBases { dpx, dpy, dpz, n: points.len(), stage, h2d: t.elapsed().as_secs_f64() })
}
...
pub fn bucket_pass_staged(
    ...
    let (didx, doff) = (upload_u32(ctx, &idx)?, upload_u32(ctx, &off)?);
    let (drx, dry, drz) = (
        alloc(ctx, g.nb * 8 * 4)?,
        alloc(ctx, g.nb * 8 * 4)?,
        alloc(ctx, g.nb * 8 * 4)?,
    ...
    for d in [&drx, &dry, &drz] {
        ctx.memset_u8(d, 0xA5)
            .map_err(|e| crate::error::Error::Cuda(e.to_string()))?;
    }
    sync(ctx)?;
    ...
    let t = Instant::now();
    let (grid, block) = bucket_launch_geometry(g);
    // NOT `device::launch`, whose generic 256-thread block is right for the
    // elementwise kernels and wrong for this one. NCU says the bucket kernel is
    // LATENCY-bound - 70% of stalls are `wait`, DRAM under 9%, SM throughput
    // under 27% - and at 136 registers a 256-thread CTA needs 34,816 of an SM's
    // 65,536, so exactly one block fits and occupancy is capped at 16.7%.
    // Halving the block doubles both. Measured at n = 2^20, kernel time,
    // 256 -> 128: nw=20 1.43x, nw=22 1.53x, nw=25 1.28x, nw=28 1.03x.
    //
    // This library launched at 256 until the two copies of this module were
    // merged: the measurement, the knob and `Geom`'s divisibility assertion all
    // landed in the test copy, and the crate that ships kept the old geometry.
    // `Geom::new` asserts `nb % bucket_block() == 0`, so the grid is exact.
    ctx.launch(module, grid, block, 0, &args)
        .map_err(|e| crate::error::Error::Cuda(e.to_string()))?;
    sync(ctx)?;
    tm.kernel = t.elapsed().as_secs_f64();

    let t = Instant::now();
    let read = |d: &DeviceBuffer| -> Result<Vec<Fq>> {
        Ok(from_planar(&read_u32(ctx, d, g.nb * 8)?, g.nb))
    };
    let r_inv = r.inverse().expect("R is invertible");
