...
impl PtxEmitter {
    ...
    fn emit_u32_init(&mut self, dst: &str, expr: &Expr) {
        match expr {
            Expr::IntLit(val, _) if *val >= 0 && *val <= u32::MAX as i64 => {
                writeln!(&mut self.ptx_buffer, "    mov.u32 {}, {};", dst, *val as u32).unwrap();
            }
            _ => {
                writeln!(
                    &mut self.ptx_buffer,
    ...
    fn emit_kernel(&mut self, kernel: &KernelDecl, hw_profile: &HardwareProfile) {
        ...
        if optimal_regs > 0 {
            writeln!(&mut self.ptx_buffer, "    // [TIGHT PACK] Maximum Occupancy Target: 100% ({} warps)", hw_profile.max_warps_per_sm).unwrap();
    ...
    fn emit_block(&mut self, block: &Block, hw_profile: &HardwareProfile) {
        ...
        while i < stmts.len() {
            ...
            let is_barrier = match &stmts[i] {
                Stmt::Expr(Expr::Path { namespace, member, .. }) => {
            ...
            if is_barrier {
                let budget = (hw_profile.membar_gpu_latency_cycles / hw_profile.imad_latency_cycles) as usize;
                ...
                if hoist_count > 0 {
                    writeln!(&mut self.ptx_buffer, "    // [BARRIER HOISTING] Found barrier stall of {} cycles.", hw_profile.membar_gpu_latency_cycles).unwrap();
        ...
        match stmt {
            ...
            Stmt::For { loop_var, start, end, step, body, .. } => {
                ...
                let step_val = match step {
                    Some(Expr::IntLit(step, _)) if *step > 0 && *step <= u32::MAX as i64 => *step as u32,
                    _ => 1,
                };
            ...
            Stmt::If { condition, then_block, else_block, .. } => {
                ...
                let else_cost = else_block.as_ref().map(|b| b.stmts.len() as f64 * 1.0).unwrap_or(0.0);
                let total_cost = then_cost + else_cost;

                writeln!(&mut self.ptx_buffer, "    // [HEURISTIC] Branch Divergence Penalty is {} cycles.", hw_profile.branch_divergence_penalty_cycles).unwrap();
                if total_cost < hw_profile.branch_divergence_penalty_cycles {
                    writeln!(&mut self.ptx_buffer, "    // Block cost ({} cy) < Penalty. Emitting PREDICATED execution.", total_cost).unwrap();
                    let pred = self.alloc_pred();
                    let cond_reg = if cond_str.is_empty() { "%r_cond" } else { &cond_str };
                    writeln!(&mut self.ptx_buffer, "    setp.ne.u32 {}, {}, 0;", pred, cond_reg).unwrap();
                    ...
                    writeln!(&mut self.ptx_buffer, "    // Block cost ({} cy) >= Penalty. Emitting BRANCH execution.", total_cost).unwrap();
                    let pred = self.alloc_pred();
                    let cond_reg = if cond_str.is_empty() { "%r_cond" } else { &cond_str };
                    let else_label = self.alloc_label("IF_ELSE");
                    let end_label = self.alloc_label("IF_END");
                    writeln!(&mut self.ptx_buffer, "    setp.eq.u32 {}, {}, 0;", pred, cond_reg).unwrap();
                    if else_block.is_some() {
                        writeln!(&mut self.ptx_buffer, "    @{} bra {};", pred, else_label).unwrap();
                    } else {
    ...
    fn emit_expr(&mut self, expr: &Expr, cache_policy: Option<&CachePolicyAttr>, hw_profile: &HardwareProfile) -> String {
        match expr {
            Expr::Call { func, .. } => {
                match &**func {
                    Expr::Ident(fname, _) => {
